...
awaitable<boost::system::error_code> ssl_echo_once(ssl_socket& socket)
{
  ...
  auto [ec, bytes] = co_await socket.async_read_some(boost::asio::buffer(data), as_tuple(use_awaitable));
  if(ec) { co_return ec; }

  std::tie(ec, bytes) = co_await async_write(socket, boost::asio::buffer(data, bytes), as_tuple(use_awaitable));
